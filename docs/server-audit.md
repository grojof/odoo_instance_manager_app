...
- **System overview** — hostname, OS, kernel, architecture, virtualization, uptime, IPs, and the
  versions/paths of Python, the PostgreSQL client, and Nginx. Missing probes show a "not detected" marker.
...
The audit runs only inspection and discovery commands. It never produces or applies an install, configuration,
or deletion command — see the [read-only requirement in the spec](../openspec/specs/server-audit/spec.md).
